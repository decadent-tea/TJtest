...
export function FlowList({
  ...
  const [query, setQuery] = useSessionState<string>("flows-query", "");
  const [page, setPage] = useSessionState<number>("flows-page", 0);
  const [pageSize, setPageSize] = useSessionState<number>("flows-pageSize", 10);
  const visible = flows.filter((flow) =>
    `${flow.name} ${flow.project} ${flow.url}`
      .toLowerCase()
          ...
          <CardContent>
            <Blank
              ...
              description={
                flows.length
                  ? "调整搜索条件后重试。"
                  : "完成一次录制后，在体检记录详情选择“保存为流程”。你可以编辑步骤并添加验证条件。"
              }
              action={
                <Button
                  variant="outline"
...
export function FlowEditor({
  ...
  const [locatorText, setLocatorText] = useState(() =>
    JSON.stringify(flow.operations[0]?.locators || [], null, 2),
  );
  const [locatorError, setLocatorError] = useState("");
  const { requestClose, confirmation } = useUnsavedChanges(
    JSON.stringify(draft) !== JSON.stringify(flow) || !!locatorError,
    busy,
    onClose,
  );
  const dependencyError = draft.operations.find((step, index) =>
    step.dependsOn.some(
      (id) =>
        !draft.operations.slice(0, index).some((prior) => prior.id === id),
    ),
  );
  ...
  const choose = (o: Operation) => {
    if (locatorError) {
      toast.error("请先修正当前步骤的定位器，再切换步骤。");
      return;
    }
    setSelected(o.id);
  ...
  const add = (kind: "assert" | "goto" | "hover") => {
    ...
    const next: Operation = {
      ...
      kind,
      navigationMode: kind === "goto" ? "navigate" : undefined,
      label:
        kind === "hover"
          ? "鼠标悬浮 · 请设置父菜单定位器"
          : kind === "assert"
    ...
    const index =
      kind === "hover" && op ? operations.indexOf(op) : operations.length;
    operations.splice(index, 0, next);
    setDraft({
      ...draft,
      operations: operations.map((step, i) => ({ ...step, sequence: i + 1 })),
    });
    choose(next);
  };
  return (
          ...
          <fieldset disabled={busy} className="contents">
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button
                        ...
                        size="icon-xs"
                        aria-label="下移步骤"
                        disabled={busy || i === draft.operations.length - 1}
                        onClick={() => move(o.id, 1)}
                      >
                        <ArrowDown />
                      </Button>
                      <Button
                        ...
                        size="icon-xs"
                        aria-label="删除步骤"
                        disabled={busy || !!locatorError}
                        onClick={() => {
                          if (
                            draft.operations.some((step) =>
                              step.dependsOn.includes(o.id),
                            )
                          ) {
                            toast.error(
                              "有后续步骤依赖此步骤，请先调整依赖，或停用该步骤。",
                            );
                            return;
                          }
                          const removed = structuredClone(o);
                          toast("步骤已从草稿移除，保存后生效", {
                            action: {
                              label: "撤销",
                              onClick: () =>
                                setDraft((current) => {
                                  if (
                                    current.operations.some(
                                      (step) => step.id === removed.id,
                                    )
                                  )
                                    return current;
                                  const restored = [...current.operations];
                                  restored.splice(
                                    Math.min(i, restored.length),
                                    0,
                                    removed,
                                  );
                                  return {
                                    ...current,
                                    operations: restored.map((step, index) => ({
                                      ...step,
                                      sequence: index + 1,
                                    })),
                                  };
              ...
              <div className="flow-editor-fields">
                {op ? (
                  <FieldGroup>
                    ...
                    <FieldGroup className="grid grid-cols-2 gap-4">
                      <Field>
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            update({
                              kind: v as Operation["kind"],
                              navigationMode:
                                v === "goto" ? "navigate" : undefined,
                            })
                          }
                        >
                          <SelectTrigger>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          update({
                            ...
                            ...(op.kind === "goto"
                              ? { navigationMode: "navigate" as const }
                              : {}),
                          })
                        }
                        placeholder={
                    ...
                    <Field>
                      <FieldLabel htmlFor="op-variable">
                        运行变量名称
                      </FieldLabel>
                      <Input
                        id="op-variable"
                    ...
                    <Field>
                      <FieldLabel htmlFor="op-assertion">
                        预期页面文本
                      </FieldLabel>
                      <Input
                        id="op-assertion"
                    ...
                    <Field data-invalid={!!locatorError}>
                      <FieldLabel htmlFor="op-locators">
                        元素定位器候选
                      </FieldLabel>
                      <Textarea
                        id="op-locators"
                      ...
                      <FieldDescription>
                        {locatorError ||
                          (op.kind === "hover"
                            ? "填写需要悬浮的父菜单定位器；此步骤应位于子菜单点击之前。支持 testId、role、label、placeholder、text、css、xpath。"
                            : "按顺序尝试 testId、role、label、placeholder、text、css、xpath 定位器，仅使用唯一可见匹配。XPath 候选填写 kind 为 xpath，value 为 XPath 表达式。")}
                      </FieldDescription>
                    </Field>
            ...
            {dependencyError && (
              <p role="alert" className="text-destructive text-sm">
                “{dependencyError.label}
                ”的依赖必须位于该步骤之前，请调整顺序或依赖。
              </p>
            )}
          </fieldset>
          <DialogFooter>
