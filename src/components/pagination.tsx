...
export function Pagination({
  ...
  const go =
    (nextPage: number) => (event: React.MouseEvent<HTMLAnchorElement>) => {
      ...
      if (nextPage >= 0 && nextPage <= lastPage && nextPage !== shownPage)
  ...
  const pageNumbers = Array.from(
    ...
    (index) =>
      index === 0 || index === lastPage || Math.abs(index - shownPage) <= 1,
  );
  return (
    <div className="pagination-controls">
      <div className="pagination-settings">
        ...
        >
          <SelectTrigger size="sm" className="w-[96px]" aria-label="每页条数">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectGroup>
        ...
        <PaginationContent>
          <PaginationItem>
            <PaginationPrevious
              ...
              tabIndex={shownPage === 0 ? -1 : undefined}
              onClick={go(shownPage - 1)}
            />
          </PaginationItem>
          ...
          <PaginationItem>
            <PaginationNext
              href="#"
              aria-disabled={shownPage === lastPage}
              tabIndex={shownPage === lastPage ? -1 : undefined}
              onClick={go(shownPage + 1)}
            />
          </PaginationItem>
        </PaginationContent>
